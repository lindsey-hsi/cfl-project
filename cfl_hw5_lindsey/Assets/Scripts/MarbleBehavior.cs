...
public class MarbleBehavior : MonoBehaviour
{
    ...
    {
    Debug.LogFormat("THIS IS THE NAME: {0}", collision.gameObject.name);
        if (collision.gameObject.name.Contains("Obstacle") ||
            collision.gameObject.name.Contains("Mover"))
        {
            gameManager.Health -= 5;
            Debug.LogFormat("Marble Health: {0}", gameManager.Health);
        }
    }
